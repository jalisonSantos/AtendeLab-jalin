    ...
    <div class="table-responsive">
        <table class="table table-hover align-middle mb-0">
            <thead class="table-light">
                <tr><th>ID</th><th>Pessoa</th><th>Tipo</th><th>Responsável</th><th>Data</th><th>Status</th><th class="text-end">Ações</th></tr>
            </thead>
...
async function carregarCombos() {
    const [pessoasResp, tiposResp] = await Promise.all([AtendeLabApi.get('pessoas', 'listar'), AtendeLabApi.get('tipos', 'listar')]);
    ...
    document.getElementById('pessoaSelect').innerHTML = '<option value="">Selecione</option>' + pessoas.map(p => `<option value="${Number(p.id)}">${AtendeLabApi.escape(p.nome)}</option>`).join('');
    document.getElementById('tipoSelect').innerHTML = '<option value="">Selecione</option>' + tipos.map(t => `<option value="${Number(t.id)}">${AtendeLabApi.escape(t.nome)}</option>`).join('');
}
async function carregarAtendimentos() {
    try {
        ...
        if (!atendimentos.length) { tbody.innerHTML = '<tr><td colspan="7" class="text-center py-4">Nenhum atendimento registrado.</td></tr>'; return; }
        tbody.innerHTML = atendimentos.map(a => {
            ...
            return `<tr>
                ...
                <td class="text-end"><button class="btn btn-sm btn-outline-primary" onclick="abrirStatus(${Number(a.id)}, '${AtendeLabApi.escapeAttr(a.status)}')">Status</button></td>
            </tr>`;
        }).join('');
...
formAtendimento.addEventListener('submit', async event => {
    ...
    try {
        ...
        fecharFormulario(); await carregarAtendimentos();
    } catch (error) { AtendeLabApi.showAlert('alerta', error.message, 'danger'); }
});
...
document.addEventListener('DOMContentLoaded', async () => {
    try { await carregarCombos(); await carregarAtendimentos(); }
    catch (error) { AtendeLabApi.showAlert('alerta', error.message, 'danger'); }
});
</script>
