    ...
    <script>
        document.addEventListener('DOMContentLoaded', async () => {
            try {
                ...
                document.getElementById('totalPessoas').textContent      = ind.total_pessoas      ?? '!';
                document.getElementById('totalTipos').textContent        = ind.total_tipos        ?? '!';
                document.getElementById('totalAtendimentos').textContent = ind.total_atendimentos ?? '!';
            } catch (error) {
                ['totalPessoas', 'totalTipos', 'totalAtendimentos'].forEach(id => {
                    const el = document.getElementById(id);
                    if (el) { el.textContent = '!'; el.title = error.message; }
                });
            }
        });
    </script>
